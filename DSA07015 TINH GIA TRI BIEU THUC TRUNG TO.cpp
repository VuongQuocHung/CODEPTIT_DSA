...
// Tách chuỗi đề cho thành 1 vector<string> chứa các toán tử, toán hạng, '(', ')'
vector<string> tach_thanh_vector(string s){
	vector<string> ans;
	for(int i = 0; i < s.size(); i++){
		// lưu các toán hạng có thể có nhiều chữ số
		if(isdigit(s[i])){
			string tmp;
			...
			i += tmp.size() - 1; // trừ đi 1 vì đến vòng for tiếp theo i sẽ tăng thêm 1
			ans.pb(tmp);
		}
...
vector<string> trung_to_sang_hau_to(vector<string> v){
	stack<string> st;
	vector<string> ans;
	for(string x : v){
